...
/**
 * Hook that provides on-submit abusive text checking for project descriptions.
...
export function useAbuseCheck(options: UseAbuseCheckOptions = {}) {
  ...
  const checkText = useCallback(
    async (text: string): Promise<AbuseCheckResult> => {
      ...
      try {
        const res = await fetch("/api/moderation/check-abuse", {
          method: "POST",
        ...
        if (finalResult.abusive && userId) {
          fetch("/api/moderation/log-abuse", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              userId,
